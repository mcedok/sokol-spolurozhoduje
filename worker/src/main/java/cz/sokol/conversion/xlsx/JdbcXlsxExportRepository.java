...
public final class JdbcXlsxExportRepository implements XlsxExportProcessor.Repository {
  private final DataSource dataSource;
  private final Duration leaseDuration;
  private final Supplier<Instant> clock;

  public JdbcXlsxExportRepository(DataSource dataSource, Duration leaseDuration) {
    this(dataSource, leaseDuration, Instant::now);
  }

  JdbcXlsxExportRepository(DataSource dataSource, Duration leaseDuration, Supplier<Instant> clock) {
  ...
  @Override
  public Optional<XlsxExportProcessor.Job> claimNext() throws SQLException {
    Instant now = clock.get();
    String sql = """
        with candidate as (
          select id from xlsx_export_jobs
          where status = 'queued'
             or (status = 'processing' and lease_expires_at < ?)
          order by created_at, id
          for update skip locked limit 1
        )
        update xlsx_export_jobs job
        set status='processing', started_at=coalesce(started_at, ?),
            lease_expires_at=?, attempt_count=attempt_count+1,
            error_code=null, error_detail=null, row_version=row_version+1, updated_at=?
  ...
  @Override
  public void complete(XlsxExportProcessor.Job job, UUID fileId, String container, String key,
      String sha256, long size, String etag) throws SQLException {
    Instant now = clock.get();
    try (var connection = dataSource.getConnection()) {
      connection.setAutoCommit(false);
      try {
        try (var statement = connection.prepareStatement("""
          ...
          statement.setString(6, "pripominky-" + job.id() + ".xlsx");
        ...
        try (var statement = connection.prepareStatement("select id from file_objects where object_key=? and sha256=? for update")) {
          statement.setString(1, key);
          statement.setString(2, sha256);
          try (var rows = statement.executeQuery()) {
            if (!rows.next()) throw new SQLException("Uložený XLSX derivát nebyl nalezen.");
            persistedFileId = rows.getObject(1, UUID.class);
          }
        }
        try (var statement = connection.prepareStatement("""
            update xlsx_export_jobs
            set status='completed', output_file_id=?, completed_at=?,
          ...
          if (statement.executeUpdate() != 1) throw new SQLException("XLSX export již není ve zpracovatelném stavu.");
        }
        connection.commit();
      } catch (Exception error) {
        connection.rollback();
  ...
  @Override
  public void fail(XlsxExportProcessor.Job job, String errorCode, String detail) throws SQLException {
    Instant now = clock.get();
    try (var connection = dataSource.getConnection(); var statement = connection.prepareStatement("""
        update xlsx_export_jobs set status='failed', error_code=?, error_detail=?,
          lease_expires_at=null, row_version=row_version+1, updated_at=?
